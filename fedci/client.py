...
class ComputationHelper:
    ...
    def run_model(y, X, model):
        ...
        if type(model.family) == family.Gaussian:
            ...
            raise Exception(
                f"Cannot handle model family {model.family.__class__.__name__}"
            )
...
class CategoricalComputationUnit(ComputationUnit):
    ...
    def compute(data, y_label, X_labels, beta):
        ...
        return {"llf": llf, "deviance": deviance, "beta_update_data": results}
...
class CategoricalOVRComputationUnit(ComputationUnit):
    ...
    def compute(data, y_label, X_labels, betas):
        ...
        etas = {
            c: np.clip(m.predict(which="linear"), -350, 350) for c, m in models.items()
        }
        denom = 1 + sum(np.exp(eta) for eta in etas.values())
        mus = {
            c: np.clip(np.exp(eta) / denom, 1e-8, 1 - 1e-8) for c, eta in etas.items()
        }
        ...
        # LLF
        llf += np.sum(
            np.log(np.take(1 / denom, reference_category_indices.nonzero()[0]))
        )
...
class OrdinalComputationUnit(ComputationUnit):
    ...
    def compute(data, y_label, X_labels, betas):
        ...
        mus = [
            (level, model.predict())
            for level, model in sorted(
                models.items(), key=lambda lvl: int(lvl[0].split("__ord__")[-1])
            )
        ]
        # get diffs of mus of successive levels
        mus_diff = [mus[0]]
        mus_diff.extend(
            [(mus[i][0], mus[i][1] - mus[i - 1][1]) for i in range(1, len(mus))]
        )
...
regression_computation_map = {
    ...
    if OVR == 0
    else CategoricalOVRComputationUnit,
...
class Client:
    ...
    def compute(self, y_label: str, X_labels, beta):
        ...
        result = regression_computation_map[self.schema[y_label]].compute(
            self.expanded_data, y_label, X_labels, beta
        )
        ...
        if self.schema[y_label] in [VariableType.CONTINUOS, VariableType.BINARY]:
            response: ClientResponseData = ClientResponseData(
                ...
                beta_update_data={
                    list(beta.keys())[0]: BetaUpdateData(
                        xwx=result["xwx"], xwz=result["xwz"]
                    )
                },
            )
        else:
            beta_update_data = {
                category: BetaUpdateData(xwx=data["xwx"], xwz=data["xwz"])
                for category, data in result["beta_update_data"].items()
            }
            response: ClientResponseData = ClientResponseData(
                llf=result["llf"],
                deviance=result["deviance"],
