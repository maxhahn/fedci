...
class OrdinalTestingRound(TestingRound):

    def __init__(self, y_label, X_labels, levels):
        self.y_label = y_label
        self.X_labels = X_labels
        self.levels = levels
        self.beta = {c: np.zeros(len(self.X_labels) + 1) for c in levels}
        self.client_data = {c:self._get_client_dict() for c in levels}
        ...
        for y_var in available_data:
            ...
            for var_set in powerset_of_regressors:
                for _var, expressions in category_expressions.items():
                    if _var in var_set:
                        var_set = (set(var_set) - {_var}) | set(sorted(list(expressions)[1:])) # [1:] to drop first cat
                if mode == 'dummy':
            ...
            if y_var in category_expressions:
                ...
                self.testing_rounds.extend([OrdinalTestingRound(y_label=y_var, X_labels=sorted(list(x_vars)), levels=ordinal_expressions[y_var][:-1]) for x_vars in powerset_of_regressors])
            else:
...
class Server:
    def __init__(self, clients, max_regressors=None):
        ...
        for client in self.clients.values(): client.receive_ordinal_expressions(self.ordinal_expressions)
...
class Client:
    ...
    def receive_ordinal_expressions(self, expressions):
        self.ordinal_expressions = set([li for l in expressions.values() for li in l])
    ...
    def compute(self, y_label: str, X_labels: List[str], beta):  
        ...
        mode = 'dummy'#'dummy'
        if mode == 'dummy':
            _data = _data.to_dummies((cs.integer() - cs.by_name(y_label)), separator='__ord__')
            missing_cols += list(self.ordinal_expressions - set(_data.columns))
        ...
        if y_label in self.schema and self.schema[y_label] == VariableType.ORDINAL:
            return self._compute_ordinal(_data, y_label, X_labels, beta)
    ...
    def _compute_ordinal(self, data, y_label, X_labels, betas):
        #print(f'REGRESSING: {y_label} ~ {X_labels}')
        #print(betas)
        ...
        logreg_results = {}
        for level, beta in betas.items():
            level_int = int(level.split('__ord__')[-1])
            y = data.with_columns(pl.col(y_label) <= level_int).to_pandas()[y_label]
            y = y.to_numpy().astype(float)
            
            
            logreg_results[level] = self._run_regression(y, X, beta, True, return_model=True)
            
            # TURN INTO SORTED LIST FOR ACCES OF CONSECUTIVE ELEMENTS
            
        logreg_results_list = sorted([(k,v[1]) for k,v in logreg_results.items()], key=lambda x: x[0])   
        
        
        # LLF
        def get_cat_index(data, y_label, level):
            level_int = int(level.split('__ord__')[-1])
            return data.with_row_index().filter(pl.col(y_label) == level_int)['index'].to_list()
        
        def get_ref_cat_index(data, y_label, levels):
            level_ints = [int(level.split('__ord__')[-1]) for level in levels]
            return data.with_row_index().filter(~pl.col(y_label).is_in(level_ints))['index'].to_list()
        
        llf = logreg_results_list[0][1].llf # GET LLF FOR P(K=1)
        for i in range(len(logreg_results)-1):
            _, m0 = logreg_results_list[i]
            lvl1, m1 = logreg_results_list[i+1]
            ...
            idx = get_cat_index(self.data, y_label, lvl1)
            
            llf += np.sum(np.log(np.take(mu_diff, idx)))
        llf += np.sum(np.log(np.take(np.clip(1-mu1, 1e-15, 1-1e-15), get_ref_cat_index(self.data, y_label, logreg_results.keys()))))
        
        # DEVIANCE + LLF SAT
        llf_sat = 0
        for level in logreg_results.keys():
            level_int = int(level.split('__ord__')[-1])
            y = data.with_columns(pl.col(y_label) == level_int).to_pandas()[y_label]
            y = y.to_numpy().astype(float)
        ...
        def get_ref_cat_mask(data, y_label, levels):
            level_ints = [int(level.split('__ord__')[-1]) for level in levels]
            return data.with_columns(__mask=~pl.col(y_label).is_in(level_ints))['__mask'].cast(pl.Float64).to_numpy()
            
        # Handle reference category similarly
        #y_ref = data.to_pandas()[reference_category]
        #y_ref = y_ref.to_numpy().astype(float)
        y_ref = get_ref_cat_mask(self.data, y_label, logreg_results.keys())
        llf_sat += np.sum(y_ref * np.log(np.clip(y_ref, 1e-10, None)))
        
        deviance = 2 * (llf_sat - llf)
        
        results = {c:(v[0][0], v[0][1]) for c,v in logreg_results.items()}
        return results, llf, deviance
    ...
    def _run_regression(self, y, X, beta, do_log_reg, return_model=False):
        if do_log_reg:
        ...
        z = eta + (y - mu)/dmu_deta
        #z = np.divide((y-mu),dmu_deta,out=np.ones_like(mu)*1e+20, where=dmu_deta!=0)
        ...
        if return_model:
            return (xwx, xwz, deviance, llf, rss, len(y)), glm_results
...
class LikelihoodRatioTest:
    ...
    def _run_likelihood_test(self):
        ...
        t0_dof = len(self.t0.beta)#-1 + 1 # + intercept
        t1_dof = len(self.t1.beta)#-1 + 1 # + intercept
        
        #print(f'X_labels for t0: {self.t0.X_labels}, {self.t0.beta}')
        #print(f'X_labels for t1: {self.t1.X_labels}, {self.t1.beta}')
        
        p_val = scipy.stats.chi2.sf(t, t1_dof-t0_dof)
    ...
    def __lt__(self, x):
        if len(self.s_labels) < len(x.s_labels):
            return True
        elif len(self.s_labels) > len(x.s_labels):
            return False
        
        if self.y_label < x.y_label:
            return True
        elif self.y_label > x.y_label:
            return False
            
        if self.x_label < x.x_label:
            return True
        elif self.x_label > x.x_label:
            return False
        
        if tuple(sorted(self.s_labels)) < tuple(sorted(x.s_labels)):
            return True
        elif tuple(sorted(self.s_labels)) > tuple(sorted(x.s_labels)):
            return False
...
#         print(f'OrdinalTest: {self.y_label} ~ {self.x_label} + {self.s_labels}')
#         print(f'LLFs: {t0_llf}, {t1_llf}')
#         print(f'DOFs: {t0_dof}, {t1_dof}')
#         print(f'Stats: {t}, {p_val}')
...
def get_likelihood_tests(finished_rounds):
    ...
    for curr_round in finished_rounds:
        ...
        for x_var in curr_X:
            ...
            elif type(curr_round) == OrdinalTestingRound:
                tests.append(CategoricalLikelihoodRatioTest(comparison_round, curr_round))
            else:
