...
df = df.explode('predicted_p_values', 'baseline_p_values')
...
df_correlation_fix = df.with_columns(correct=(pl.col('predicted_p_values') - pl.col('baseline_p_values')).round(8) == 0)
...
df_correlation_fix2 = df.with_columns(
    pl.n_unique('predicted_p_values', 'baseline_p_values').over('name', 'num_clients', 'num_samples').name.suffix('_nunique')
)
df_correlation_fix2 = df_correlation_fix2.filter((pl.col('predicted_p_values_nunique') == 1) | (pl.col('baseline_p_values_nunique') == 1))
df_correlation_fix2 = df_correlation_fix2.drop('predicted_p_values_nunique', 'baseline_p_values_nunique')
df_correlation_fix2 = df_correlation_fix2.group_by('name', 'num_clients', 'num_samples').agg(
    mean_correctness=(pl.col('predicted_p_values')==pl.col('baseline_p_values')).mean(),
    mean_difference_p_value=(pl.col('predicted_p_values').mean()-pl.col('baseline_p_values').mean()).abs()
)
...
_df = _df.group_by(
    ...
    p_value_correlation=pl.corr('predicted_p_values', 'baseline_p_values')
)
...
_df = _df.with_columns(
    tp=(pl.col('predicted_p_values') < alpha) & (pl.col('baseline_p_values') < alpha),
    tn=(pl.col('predicted_p_values') > alpha) & (pl.col('baseline_p_values') > alpha),
    fp=(pl.col('predicted_p_values') < alpha) & (pl.col('baseline_p_values') > alpha),
    fn=(pl.col('predicted_p_values') > alpha) & (pl.col('baseline_p_values') < alpha),
)
...
#_df = df
#_df = _df.unpivot(['predicted_p_values', 'baseline_p_values'], index=['name', 'experiment_type', 'conditioning_type', 'num_clients', 'num_samples'])
...
#_df = _df.rename({'predicted_p_values': 'predicted', 'baseline_p_values': 'true'})
#_df = _df.with_columns(
